...
Perhaps nothing is wrong. It works as intended. Perhaps it is not
as fast as we could get with one of the packages build on Rcpp.

This one certainly appears to be slow because of all of the object
copying. If we could simply insert a column into a data.frame without
re-writing it, this would surely go faster.  The fear of excess data
copying is what fuels innovations during the past 5-10 years, such as
the packages ```data.table``` ```plyr``` and ```dplyr```.
...
Hadley Wickham has proposed a number of R packages and new
idioms. Like data.table, his plyr package, and his successor dplyr,
are intended to make R code both more "elegant" (in the eyes of the
author, at least), faster, and less error prone.  The plyr package 
implements an approach that is described as the "split, apply,
...
I do not know if mutate suffers from the same problems that makes
transform so worrisome.

```{r}
library(dplyr)
hsb %>% group_by(schoolidf) %>% summarize(ses_mn = mean(ses, na.rm = TRUE)) %>% right_join(hsb, by = "schoolidf")
hsbdplyr <- hsb %>% group_by(schoolidf) %>% mutate(ses_mn = mean(ses, na.rm=TRUE))
hsbdplyr[ , c('schoolidf', 'ses', 'ses_mn')]
hsbdplyr$ses_dev <- hsbdplyr$ses - hsbdplyr$ses_mn
hsbdplyr[ , c('schoolidf', 'ses', 'ses_mn', 'ses_dev')]
```
